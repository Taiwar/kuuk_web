...
export default class CacheHelper {
  ...
  private static writeItemToCache(
  ...
  static addItem(
    cache: ApolloCache<any>,
    itemDTO: IngredientDTO | StepDTO | NoteDTO,
    itemType: GroupItemTypes,
  ): void {
    cache.modify({
  ...
  static removeItem(
    cache: ApolloCache<any>,
    deletionResponse: GroupItemDeletionResponse,
    itemType: GroupItemTypes,
  ): void {
    cache.modify({
      ...
      fields: {
        items(existingItems = []) {
          if (deletionResponse.success) {
            const itemRemoved = existingItems.filter(
              (i: { __ref: string }) =>
                CacheHelper.buildItemCacheId(deletionResponse.id, itemType) !==
                i.__ref,
            );
              ...
              cache.modify({
                ...
                fields: {
                  sortNr(itemSortNr: number) {
                    return ReorderingHelper.sortNrAfterDeletion(
                      itemSortNr,
                      deletionResponse.sortNr,
                    );
                  },
                },
              });
  ...
  static updateItem(
    cache: ApolloCache<any>,
    updateItemResponse: GroupItemUpdateResponse,
    itemType: GroupItemTypes,
  ): void {
    cache.modify({
      ...
      fields: {
        items(existingItems = []) {
          const itemCacheId = CacheHelper.buildItemCacheId(
            updateItemResponse.item.id,
            itemType,
          );
          const oldItemRemoved = existingItems.filter(
            (i: { __ref: string }) => itemCacheId !== i.__ref,
          );
          const ref = CacheHelper.writeItemToCache(
            cache,
            updateItemResponse.item,
            itemType,
          );
          const newSortNr = updateItemResponse.item.sortNr;
          const isMovingGroups = (updateItemResponse.item.groupID &&
            updateItemResponse.prevGroupID &&
            updateItemResponse.prevGroupID.toString() !==
              updateItemResponse.item.groupID.toString()) as boolean;
          ...
          for (const item of oldItemRemoved) {
            cache.modify({
              ...
              fields: {
                sortNr(itemSortNr: number) {
                  const resultNr = ReorderingHelper.sortNrAfterReorder(
                    itemSortNr,
                    newSortNr,
                    isMovingPosition,
                    isMovingGroups,
                    updateItemResponse.prevSortNr,
                  );
                  if (resultNr < 1) {
          ...
          if (ref === undefined) {
            console.error(
              'Updated item ref was undefined! Falling back to original state.',
            );
            return existingItems;
          } else {
