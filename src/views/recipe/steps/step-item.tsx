...
export function StepItem(props: StepItemProps): JSX.Element {
  ...
  return (
    <div key={step.id} className="grid grid-cols-12 max-w-3xl">
      <button
        hidden={!editable}
        className="col-span-1 h-8 w-8 rounded-full px-2 my-0.5 shadow bg-pink-400 text-white mr-1"
      ...
      <form className="col-span-11" onSubmit={handleSubmit(onSubmit)}>
        <div
          className="grid grid-cols-11 justify-center"
          onDoubleClick={handleOnFormDoubleClick}
        >
          <span className="col-span-1 inline-block bg-pink-400 text-white text-center rounded-full mb-1 mr-2 w-7 h-7">
            {step.sortNr}
          </span>

          <span
            className={`col-span-4 font-bold ${fieldsEditable ? 'hidden' : ''}`}
          >
            {step.name}
          </span>
          <input
            ...
            required
            className={`col-span-3 mr-2 h-12 rounded-md border-gray-300 shadow-sm ${
          ...
          <p
            className={`col-span-6 inline ml-1 ${
              fieldsEditable ? 'hidden' : ''
            }`}
          >
            {step.description}
          ...
          <textarea
            className={`col-span-5 rounded-md border-gray-300 shadow-sm ${
              !fieldsEditable ? 'hidden' : ''
            }`}
