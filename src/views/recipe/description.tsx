...
export function RecipeDescription(props: { recipe: RecipeDTO }) {
  const { recipe } = props
  ...
  return <div>
    <div className="flex">
      ...
      <div hidden={showForm || recipe.description !== null} className="ml-2 mt-3">
        <button className="rounded-full p-1 shadow bg-pink-400 text-white" onClick={handleClickAdd}>
          <Plus size={24}/>
        </button>
      </div>
    </div>
    <div className="mb-4" hidden={showForm} onDoubleClick={() => setShowForm(true)}>
      <p>{ recipe.description }</p>
    </div>
    <form ref={formRef} hidden={!showForm} onSubmit={handleSubmit(handleOnSubmit)}>
      <div className="grid grid-cols-4 gap-1 lg:w-1/2">
        <div className="col-span-3">
          <textarea required className="block w-full rounded-md border-gray-300 shadow-sm" placeholder="Description*" {...register('description')} />
        </div>
        <div className="col-span-1">
          <button className="rounded-md shadow-lg bg-pink-300 p-2 text-white fw-bold" type="submit">Update</button>
          <button className="rounded-md shadow-lg bg-pink-100 p-2 text-gray-600 fw-bold ml-2" onClick={handleCancel}>Cancel</button>
        </div>
      </div>
    </form>
  </div>
